...
<body id="page-top">
        ...
        <div id="content-wrapper" class="d-flex flex-column">
            ...
            <div id="content">
                ...
                <div class="container-fluid">
                    ...
                    <!-- Content Row -->
                    <div class="row">
                        <form action="database/add_mlt_appointment_script.php" method="POST" class="row g-3">
                            <div class="col-md-12">
                                <label for="mlt_name" class="form-label" >Test Name</label>
                            <!-- <input type="text" class="form-control" name="pat_name" id="pat_name"> -->
                                <select class="form-control" name="test_name" id="test_name" onchange="getdata();">
                            ...
                            <div class="col-md-12">
                                <label for="pat_email" class="form-label"></label>
                                <?php
                                    if(isset($_GET['action']) && isset($_GET['id'])){
                                        if($_GET['action'] == 'searchSchedule'){
                                            $s = $conn->prepare('select * from mlt_schedule where mlt_id = ?');
                                            $s->bind_param('i',$_GET['id']);
                                            $s->execute();
                                            $r = $s->get_result();
                                            while($ro = $r->fetch_assoc()){
                                ?>
                                            <label><?php echo $ro['day']?></label>
                                <?php   }
                                    }
                                }
                                ?>
    ...
    <script>
        function getdata(){
            ...
            $.ajax({
                type: "POST",
                url: "test_schedule_fetch.php",
                dataType: "html",
                data: {
                        'test_name': test_name
                    },
                success: function (response) {  
                        $('#mlt_schedule').html(response);
                    }
            });
        }
    </script>
